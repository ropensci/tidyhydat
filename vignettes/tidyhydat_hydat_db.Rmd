...
```{r, include=FALSE}
# unbeknownst to the reader, we are actually going to surreptitiously use the test database.
prev_default <- tidyhydat:::hy_set_default_db(hy_test_db())
```

## Working with HYDAT tables

The HYDAT database is a SQLite database, which can be accessed in R using the [dplyr package](https://db.rstudio.com/dplyr/). This package has simplified the connection process, so all you have to do to connect to the database is use `hy_src()`.

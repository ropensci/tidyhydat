...
The `realtime_ws()` function provides access to ECCC's real-time web service. This offers several advantages including access to a wider variety of parameters and the ability to filter by date range. Using `realtime_ws()` requires specifying both the station and parameter of interest:
...
You can query multiple parameters simultaneously. See `data("param_id")` for a complete list of available parameter codes:
...
## Historical web service functions
For smaller queries where downloading the entire HYDAT database is unnecessary, `tidyhydat` provides web service functions that access historical daily data directly from ECCC. These functions are particularly useful when you need data for only a few stations or a limited time period:

### Historical flows and levels - `ws_daily_flows()` and `ws_daily_levels()`
These functions download historical daily data without requiring HYDAT. Both require a date range to be specified:

``` r
ws_daily_flows(station_number = "08MF005",
...
``` r
ws_daily_levels(station_number = "08NL071",
...
``` r
ws_daily_flows(station_number = c("08MF005", "08NL071"),
               start_date = "2020-01-01",
               end_date = "2020-12-31")
```
...
##  2 08MF038        FRASER RIVER AT CANNOR                 BC                      49.1     -122.
##  3 08MF040        FRASER RIVER ABOVE TEXAS CREEK         BC                      50.6     -122.
##  4 08MF062        COQUIHALLA RIVER BELOW NEEDLE CREEK    BC                      49.5     -121.
##  5 08MF065        NAHATLATCH RIVER BELOW TACHEWANA CREEK BC                      50.0     -122.
##  6 08MF068        COQUIHALLA RIVER ABOVE ALEXANDER CREEK BC                      49.4     -121.
##  7 08MF001        ANDERSON RIVER NEAR BOSTON BAR         BC                      49.8     -121.
##  8 08MF002        BOULDER CREEK NEAR LAIDLAW             BC                      49.3     -122.
##  9 08MF003        COQUIHALLA RIVER NEAR HOPE             BC                      49.4     -121.
## 10 08MF004        FRASER RIVER ABOVE THOMPSON RIVER      BC                      50.2     -122.
## # ℹ 44 more rows
```
...
![plot of chunk unnamed-chunk-13](vignette-fig-unnamed-chunk-13-1.png)

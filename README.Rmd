...
- Provides functions (`ws_*`) that access historical daily data via web service without requiring HYDAT download.
...
tidyhydat
`'s function will look for it. Do be patient though as this can take a long time! To see where HYDAT was saved you can run `
hy_default_db()
...
For smaller queries where downloading the entire HYDAT database is unnecessary, `tidyhydat` provides web service functions that access historical daily data directly:

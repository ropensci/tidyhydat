...
hy_sed_daily_loads <- function(
  ...
  sed_dly_loads <- tidyr::gather(
    sed_dly_loads,
    !!sym_variable,
    !!sym_temp,
    -(STATION_NUMBER:NO_DAYS)
  )
  sed_dly_loads <- dplyr::mutate(
    sed_dly_loads,
    DAY = as.numeric(gsub("LOAD", "", variable))
  )
  sed_dly_loads <- dplyr::mutate(
    sed_dly_loads,
  ...
  if (!dates_null[["end_is_null"]])
    sed_dly_loads <- dplyr::filter(sed_dly_loads, !!sym_Date <= end_date)
  ...
  attr(sed_dly_loads, "missed_stns") <- setdiff(
    unique(stns),
    unique(sed_dly_loads$STATION_NUMBER)
  )
  as.hy(sed_dly_loads)
}

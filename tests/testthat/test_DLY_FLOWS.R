...
test_that("DLY_FLOWS accepts single and multiple province arguments", {
  stns <- "08MF005"
  expect_identical( unique(DLY_FLOWS(STATION_NUMBER = stns, hydat_path = "H:/Hydat.sqlite3")$STATION_NUMBER), stns)
  expect_identical( length(unique(DLY_FLOWS(STATION_NUMBER = c("08MF005","08KH001"), hydat_path = "H:/Hydat.sqlite3")$STATION_NUMBER)),length(c("08NM083","08NE102")))
})


test_that("DLY_FLOWS accepts single and multiple province arguments", {
  expect_true( nrow(DLY_FLOWS(PROV_TERR_STATE_LOC = "AK", hydat_path = "H:/Hydat.sqlite3")) >= 1)
  expect_true( nrow(DLY_FLOWS(PROV_TERR_STATE_LOC = c("ID","AK"), hydat_path = "H:/Hydat.sqlite3")) >= 1)
})

test_that("DLY_FLOWS produces an error when a province is not specified correctly",{
  expect_error(DLY_FLOWS(PROV_TERR_STATE_LOC = "BCD", hydat_path = "H:/Hydat.sqlite3"))
  expect_error(DLY_FLOWS(PROV_TERR_STATE_LOC = c("ID","BCD"), hydat_path = "H:/Hydat.sqlite3"))
})

## Too much data 
#test_that("DLY_FLOWS gather data when no arguments are supplied",{
#  expect_true(nrow(DLY_FLOWS(hydat_path = "H:/Hydat.sqlite3")) >= 1)
#})

test_that("DLY_FLOWS can accept both arguments for backward compatability",{
  expect_true(nrow(DLY_FLOWS(PROV_TERR_STATE_LOC = "BC", STATION_NUMBER = "08MF005", hydat_path = "H:/Hydat.sqlite3")) >= 1)
})
...
test_that("DLY_FLOWS correctly parses leaps year",{
  expect_warning(DLY_FLOWS(PROV_TERR_STATE_LOC = "BC", hydat_path = "H:/Hydat.sqlite3",

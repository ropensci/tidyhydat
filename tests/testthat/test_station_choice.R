test_that("Outputs that same station that is inputted in outputted when province is missing", {
  ...
  stns_out <- tidyhydat:::station_choice(
    hydat_con,
    station_number = stns,
...
test_that("station name in any case is accepted", {
  ...
  on.exit(DBI::dbDisconnect(hydat_con), add = TRUE)
  expect_silent(
    out_stns <- tidyhydat:::station_choice(
      hydat_con,
      station_number = stns,
      prov_terr_state_loc = NULL
    )
...
test_that("province in any case is accepted", {
  ...
  expect_identical(
    toupper(prov),
    unique(hy_stations(hydat_path, station_number = stns)$PROV_TERR_STATE_LOC)
  )
})
